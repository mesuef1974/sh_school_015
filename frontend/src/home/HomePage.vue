<template>
  <section class="d-grid gap-3">
    ...
    <div class="auto-card p-2 d-flex align-items-center gap-2 flex-wrap">
      <DsButton size="sm" :variant="reorderMode ? 'primary' : 'outline'" icon="solar:arrows-up-down-bold-duotone" @click="toggleReorder">
        {{ reorderMode ? 'إنهاء الترتيب' : 'وضع الترتيب' }}
      </DsButton>
      <div class="vr d-none d-sm-block"></div>
      <DsButton size="sm" variant="outline" :disabled="!selectedId" icon="solar:scissors-bold-duotone" @click="onCut">قص</DsButton>
      <DsButton size="sm" variant="outline" :disabled="!selectedId" icon="solar:copy-bold-duotone" @click="onCopy">نسخ</DsButton>
      <DsButton size="sm" variant="success" :disabled="!canPaste" icon="solar:clipboard-list-bold-duotone" @click="onPaste">لصق</DsButton>
...
function loadOrder() {
  ...
  } catch { orderIds.value = []; }
}
function saveOrder() {
  try { localStorage.setItem(STORAGE_KEY.value, JSON.stringify(orderIds.value)); } catch {}
}

onMounted(() => { loadOrder(); });
watch(visibleTiles, () => { /* ensure order still valid when roles change */
...
const orderedTiles = computed(() => {
  ...
  const result: any[] = [];
  for (const id of orderIds.value) { if (map.has(id)) result.push({ ...(map.get(id) as any), __key: id + ':ord' }); }
  // Append any new items not in order list at the end
  base.forEach(t => { if (!orderIds.value.includes(t.id)) result.push(t); });
  // If clipboard copy requires a preview duplicate (optional): ignore for now to keep minimal
  return result;
});

function indexOfKey(idx: number): number { return idx; }

function toggleReorder() { reorderMode.value = !reorderMode.value; }
function onTileClick(id: string) { selectedId.value = id; }
...
function onDragStart(key: string, index: number, e: DragEvent) {
  ...
  dragKey.value = key; dragFromIndex.value = index;
...
function onDrop(index: number, e: DragEvent) {
  ...
  if (from == null) return;
  moveItem(from, index);
  dragFromIndex.value = null; dragKey.value = null;
}
...
function onPaste() {
  ...
  if (clipboard.value.cut) {
    ...
    orderIds.value = list; saveOrder(); clipboard.value = null;
  } else {
    // Copy: keep unique by just moving as well (no duplicates to avoid confusion)
    if (existingIdx >= 0) { list.splice(existingIdx, 1); }
    list.splice(targetIdx, 0, srcId);
    orderIds.value = list; saveOrder();
  }
}
function resetOrder() { orderIds.value = []; saveOrder(); }
...
/* Reorder visuals */
.tile-grid.reordering .tile-wrap { cursor: grab; }
.tile-grid.reordering .tile-wrap:active { cursor: grabbing; }
.tile-wrap.selected .tile { outline: 2px solid rgba(138,21,56,0.35); outline-offset: 2px; }
</style>
