<template>
  <section class="d-grid gap-3 timetable-page">
    ...
    <DsCard
      ...
      ref="timetableRef"
      :noPadding="true"
      class="p-0 overflow-hidden timetable-card outlined-card"
    >
      <!-- Actions toolbar -->
      <div class="timetable-toolbar">
        ...
        >
          ...
          <span class="d-none d-sm-inline">{{
            isFullscreen ? "إغلاق ملء الشاشة" : "ملء الشاشة"
          }}</span>
        </button>
      ...
      <div v-if="showPrefs" class="prefs-panel">
        <div class="prefs-header">
          ...
          <button
            class="tt-btn"
            @click="showPrefs = false"
            title="إغلاق"
            aria-label="إغلاق إعدادات الجدول"
          >
              ...
              <tbody>
                <tr
                  ...
                  :enter="{
                    opacity: 1,
                    x: 0,
                    transition: { duration: 400, delay: 250 + idx * 80 },
                  }"
                >
...
.day-cell {
  background: linear-gradient(90deg, #fff, #fff9f8);
  font-weight: 600;
  color: #4a2b2b;
}
.today-cell {
  background: linear-gradient(90deg, #fff5f3, #fff);
  border-inline-start: 3px solid var(--maron-accent);
}
...
.outlined-card {
  border: 2px solid var(--maron-primary, #7b1e1e);
  border-radius: 12px;
}
...
.tt-toggle input {
  accent-color: var(--maron-primary, #7b1e1e);
}
...
.timetable-wrapper.dense .period-cell {
  padding: 0.45rem 0.4rem;
}
.timetable-wrapper.dense .subject-badge {
  font-size: 0.8rem;
}
...
.timetable-page .timetable-modern {
  font-size: 0.95rem;
  min-width: unset;
}
...
.timetable-page .period-cell {
  padding: 0.5rem 0.45rem;
}
.timetable-page .empty-period {
  min-height: 46px;
}

/* Reduce min-widths to avoid horizontal overflow */
.timetable-page .day-column {
  min-width: 110px;
}
.timetable-page .period-column {
  min-width: 120px;
}

/* Mini cards compact */
.timetable-page .mini-table thead th {
  padding: 0.4rem 0.5rem;
}
.timetable-page .mini-table tbody td {
  padding: 0.45rem 0.5rem;
}
.timetable-page .mini-stats {
  gap: 0.4rem;
}
.timetable-page .mini-stat {
  gap: 0.3rem;
  padding: 0.35rem 0.4rem;
}
.timetable-page .live-clock {
  font-size: 1rem;
}
...
function updateNowClock() {
  ...
  try {
    liveHijri.value = new Intl.DateTimeFormat("ar-SA-u-ca-islamic-nu-latn", {
      day: "numeric",
      month: "long",
      year: "numeric",
    }).format(now);
    ...
    liveHijri.value = new Intl.DateTimeFormat("ar-SA-u-nu-latn", {
      day: "numeric",
      month: "long",
      year: "numeric",
    }).format(now);
  ...
  liveTime.value = new Intl.DateTimeFormat("ar-SA-u-nu-latn", {
    hour: "2-digit",
    minute: "2-digit",
    second: "2-digit",
    hour12: false,
...
onMounted(() => {
  ...
  try {
    _clockTimer = setInterval(updateNowClock, 1000);
  } catch (_) {}
  // Auto fullscreen if enabled
  try {
    if (prefs.autoFullscreen && !isFullscreen.value) toggle();
  } catch (_) {}
...
@keyframes pulse {
  ...
  50% {
    box-shadow:
      ...
      inset 0 0 30px rgba(40, 167, 69, 0.25);
  }
}
