  ...
  <section class="d-grid gap-3 page-grid page-grid-wide" dir="rtl">
    <div class="d-flex align-items-center gap-2 mb-2 header-bar frame">
      ...
      <div>
        ...
        <div class="text-muted small" v-else>حساب O/X وفق السياسة وإصدار تنبيهات الغياب</div>
      </div>
    </div>
      ...
      <div class="card-body">
        ...
        <div class="row g-3 align-items-end">
          ...
          <div class="col-sm-2 d-flex gap-2">
            <button class="btn btn-primary flex-fill" :disabled="busy || !canCompute" @click="onCompute">
              <Icon name="mdi:calculator" class="me-1" /> احسب
            </button>
            <button class="btn btn-success flex-fill" :disabled="busy || !canIssue" @click="onIssue">
              <Icon name="mdi:file-document" class="me-1" /> إصدار تنبيه
            </button>
          </div>
        </div>
    ...
    <div v-if="alert" class="card">
      <div class="card-body">
        ...
        <a class="btn btn-outline-primary" :href="docxHref" target="_blank" rel="noopener">
          <Icon name="mdi:download" class="me-1" /> تنزيل ملف Word
        </a>
      </div>
    </div>
  </section>
...
async function loadStudents() {
  try {
    const res = await getWingStudents({ q: studentQuery.value || undefined });
...
const canCompute = computed(() => !!form.value.student && !!form.value.from && !!form.value.to);
