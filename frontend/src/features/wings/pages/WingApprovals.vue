    ...
    <div class="auto-card p-3">
      ...
      <template v-else>
        <div class="d-flex align-items-center gap-2 mb-2 flex-wrap">
          ...
          <DsBadge v-if="filteredItems.length !== items.length" :text="`بعد الفلترة: ${filteredItems.length}`" />
          <span class="ms-auto"></span>
          <div v-if="selectedClassId" class="filters d-flex align-items-center gap-2 flex-wrap me-auto">
            <div class="d-flex align-items-center gap-1">
              <span class="small text-muted">الحالة:</span>
              <label class="form-check form-check-inline small" v-for="s in statusOptions" :key="s.value">
                <input class="form-check-input" type="checkbox" :value="s.value" @change="onStatusToggle($event, s.value)" :checked="filters.statuses.has(s.value)" />
                <span class="form-check-label">{{ s.label }}</span>
              </label>
            </div>
            <div class="d-flex align-items-center gap-1">
              <span class="small text-muted">حصة:</span>
              <select class="form-select form-select-sm w-auto" v-model.number="filters.period" @change="savePrefs">
            ...
            <div class="d-flex align-items-center gap-1">
              ...
              <select class="form-select form-select-sm w-auto" v-model="filters.teacher" @change="savePrefs">
            ...
            <div class="d-flex align-items-center gap-1">
              ...
              <input type="search" class="form-control form-control-sm" style="min-width: 160px" v-model.trim="filters.q" @input="savePrefs" placeholder="اسم الطالب/ملاحظة" />
            </div>
            <button class="btn btn-sm btn-outline-secondary" type="button" @click="resetFilters">إعادة الضبط</button>
          </div>
          <details class="ms-2">
            ...
            <div class="dropdown-card p-2 mt-1">
              ...
              <label class="form-check small" v-for="(v,k) in columnVis" :key="k">
                <input class="form-check-input" type="checkbox" v-model="(columnVis as any)[k]" @change="savePrefs" />
          ...
          <DsButton :disabled="!selectedIds.size" variant="success" icon="solar:check-circle-bold-duotone" @click="decide('approve')">اعتماد المحدد</DsButton>
          <DsButton :disabled="!selectedIds.size" variant="danger" icon="solar:close-circle-bold-duotone" @click="decide('reject')">رفض المحدد</DsButton>
        </div>
        <div class="table-responsive table-card">
          <table class="table table-hover align-middle">
            <thead>
              <tr>
                <th style="width:36px"><input type="checkbox" @change="toggleAll($event)" :checked="allChecked" /></th>
                <th v-if="columnVis.student" role="button" @click="toggleSort('student')">الطالب <span class="sort" v-if="sortKey==='student'">{{ sortDir==='asc'?'▲':'▼' }}</span></th>
                <th v-if="columnVis.class" role="button" @click="toggleSort('class')">الصف <span class="sort" v-if="sortKey==='class'">{{ sortDir==='asc'?'▲':'▼' }}</span></th>
                <th v-if="columnVis.period" role="button" @click="toggleSort('period')">الحصة <span class="sort" v-if="sortKey==='period'">{{ sortDir==='asc'?'▲':'▼' }}</span></th>
                <th v-if="columnVis.status" role="button" @click="toggleSort('status')">الحالة <span class="sort" v-if="sortKey==='status'">{{ sortDir==='asc'?'▲':'▼' }}</span></th>
                <th v-if="columnVis.teacher" role="button" @click="toggleSort('teacher')">المعلم <span class="sort" v-if="sortKey==='teacher'">{{ sortDir==='asc'?'▲':'▼' }}</span></th>
                <th v-if="columnVis.note" role="button" @click="toggleSort('note')">الملاحظة <span class="sort" v-if="sortKey==='note'">{{ sortDir==='asc'?'▲':'▼' }}</span></th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="it in filteredAndSorted" :key="it.id">
                ...
                <td v-if="columnVis.status"><span class="badge text-bg-secondary">{{ statusLabel(it.status) }}</span></td>
                <td v-if="columnVis.teacher">{{ it.teacher_name || '—' }}</td>
                <td v-if="columnVis.note" class="small text-muted">{{ it.note || '—' }}</td>
              </tr>
              <tr v-if="!filteredItems.length">
                <td :colspan="1 + visibleColumnsCount" class="text-center text-muted">لا توجد عناصر مطابقة</td>
              </tr>
            </tbody>
          </table>
        </div>
...
import { getWingPending, postWingDecide, getTeacherClasses, getWingMe, getWingTimetable } from '../../../shared/api/client';
...
const wingLabel = computed(() => {
  ...
  if (names && names.length) {
    ...
    if (m) {
      const num = m[1]; const name = (m[2] || '').trim(); label = `${name} (${num})`;
    } else {
      m = raw.match(/(.+)\s*[-–]\s*الجناح\s*(\d+)/);
      if (m) { const name = (m[1] || '').trim(); const num = m[2]; label = `${name} (${num})`; }
      else { m = raw.match(/الجناح\s*(\d+)/); if (m) label = `(${m[1]})`; else label = raw; }
    }
  } else if (ids && ids.length) { label = `(${ids[0]})`; }
  return label;
});
...
const columnVis = ref<{ student: boolean; class: boolean; period: boolean; status: boolean; teacher: boolean; note: boolean }>({
  student: true,
  class: true,
  period: true,
  status: true,
...
const teachers = computed<string[]>(() => {
  ...
  for (const it of items.value) {
    if (it.teacher_name) set.add(String(it.teacher_name)); else set.add('');
  }
...
const filteredItems = computed<any[]>(() => {
  ...
  return items.value.filter(it => {
    ...
    if (q) {
      const hay = `${it.student_name || ''} ${it.note || ''} ${it.subject_name || ''}`.toLowerCase();
...
const allChecked = computed(() => filteredItems.value.length > 0 && checkedList.value.length === filteredItems.value.length);
...
const filteredAndSorted = computed<any[]>(() => {
  ...
  const val = (it: any) => {
    switch (key) {
      case 'student': return it.student_name || String(it.student_id || '');
      case 'class': return it.class_name || String(it.class_id || '');
      case 'period': return it.period_number ?? 0;
      case 'status': return statusLabel(it.status);
      case 'teacher': return it.teacher_name || '';
...
function onStatusToggle(ev: Event, s: string) {
  ...
  if (chk) filters.value.statuses.add(s); else filters.value.statuses.delete(s);
  ...
  try {
    ...
    if (!classes.value.length && Array.isArray(res.items)) {
      ...
      for (const it of res.items) {
        if (it.class_id && !map.has(it.class_id)) map.set(it.class_id, { id: it.class_id, name: it.class_name });
      }
      classes.value = Array.from(map.values()).sort((a, b) => ('' + (a.name || a.id)).localeCompare('' + (b.name || b.id), 'ar'));
    }
  } catch (e: any) {
    try { toast.error(e?.response?.data?.detail || 'تعذر تحميل الطلبات'); } catch {}
  } finally {
    loading.value = false;
  }
}
...
async function decide(action: 'approve'|'reject') {
  ...
  try {
    ...
    try { toast.error(e?.response?.data?.detail || 'فشل تنفيذ العملية'); } catch {}
  }
}
...
async function loadWingClasses() {
  ...
  try {
    if (wingId.value) {
      const res: any = await getWingTimetable({ mode: 'daily', wing_id: wingId.value, date: dateStr.value });
      const items = (res as any)?.items || [];
      const uniq = new Map<number, { id: number; name?: string | null }>();
      for (const it of items) { if (it.class_id && !uniq.has(it.class_id)) uniq.set(it.class_id, { id: it.class_id, name: it.class_name }); }
      classes.value = Array.from(uniq.values()).sort((a, b) => ('' + (a.name || a.id)).localeCompare('' + (b.name || b.id), 'ar'));
    }
  } catch {}
  ...
  if (!classes.value.length) {
    try {
      ...
      classes.value = (res?.classes || []).sort((a: any, b: any) => ('' + (a.name || a.id)).localeCompare('' + (b.name || b.id), 'ar'));
    } catch {}
  }
}
...
.table-card { margin-top: .5rem; }
.header-bar { align-items: center; }
.wing-title { color: #6a1b1b; font-weight: 700; font-size: 1.15rem; }
</style>