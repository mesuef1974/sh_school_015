<template>
  <section class="wing-dashboard d-grid gap-3 page-grid">
    <div class="auto-card p-3 d-flex align-items-center gap-3">
      ...
      <div>
        <div class="fw-bold">لوحة جناح <span v-if="wingLabel">{{ wingLabel }}</span><span v-else>—</span> — مشرف الجناح <span v-if="supervisorName">{{ supervisorName }}</span></div>
    ...
    <div v-if="!loading && kpis.total === 0" class="alert alert-warning d-flex align-items-center gap-2">
      <Icon icon="solar:info-circle-bold-duotone" />
      <div>
    ...
    <div class="row g-3">
          ...
          <div class="col-12">
            <div class="auto-card p-2 px-3 mb-1 d-flex flex-wrap align-items-center gap-2 class-toolbar">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end p-2 small" style="min-width: 260px; max-height: 300px; overflow:auto;">
                  ...
                  <li v-for="opt in classOptions" :key="opt.id" class="dropdown-item d-flex align-items-center gap-2">
                    <input class="form-check-input m-0" type="checkbox" :id="'cls-'+opt.id" :checked="selectedSet.has(opt.id)" @change="toggleClass(opt.id)" />
                    <label class="form-check-label flex-fill" :for="'cls-'+opt.id">{{ opt.name || ('صف #' + opt.id) }}</label>
                  </li>
                  <li v-if="classOptions.length === 0" class="dropdown-item text-muted">لا توجد صفوف متاحة</li>
                </ul>
              </div>
        ...
        <div class="row g-3">
          ...
          <div class="col-12">
            <div class="auto-card p-0 overflow-hidden h-100">
              ...
              <div v-if="filteredMissing.count === 0" class="p-3 text-muted">لا توجد حصص بلا إدخال اليوم في نطاق الترشيح.</div>
...
const supervisorName = computed(() => meInfo.value?.staff?.full_name || meProfile.value?.full_name || '');
...
async function safeCall<T>(fn: () => Promise<T>, key: keyof typeof apiStatus.value): Promise<T | null> {
  try {
    const res = await fn();
...
async function loadData() {
  ...
  try {
    ...
    const failedAll = [apiStatus.value.overview, apiStatus.value.missing].every((s) => s && s !== 200);
    if (failedAll) {
      error.value = 'تعذر جلب بيانات الجناح. تحقق من صلاحياتك والربط بالجناح ومن توفر واجهات /api/wing/*';
    }
  } catch (e: any) {
    error.value = e?.message || 'تعذر جلب بيانات الجناح (الشبكة أو الصلاحيات)';
  } finally {
    // Log diagnostics to help أثناء الاختبار
    try { console.debug('[WingDashboard] apiStatus', apiStatus.value); } catch {}
...
watch(selectedSet, (set) => {
  // مزامنة مع الرابط
  const ids = Array.from(set.values());
  const query = { ...route.query } as any;
  if (ids.length) query.classes = ids.join(','); else delete query.classes;
  router.replace({ query }).catch(() => {});
  // تخزين محلي
  try { localStorage.setItem('wing_selected_classes', JSON.stringify(ids)); } catch {}
}, { deep: true });
...
const kpiCards = computed(() => {
  ...
  return [
    { key: 'att_pct', label: 'نسبة الحضور', value: pct, sub: `الإجمالي ${total} | حاضر ${present}` , icon: 'solar:chart-2-bold-duotone', color: 'var(--color-success)' },
    { key: 'present', label: 'حاضر', value: present, sub: `من أصل ${total}`, icon: 'solar:check-circle-bold-duotone', color: 'var(--color-success)' },
    { key: 'absent', label: 'غائب', value: absent, sub: '', icon: 'solar:user-cross-bold-duotone', color: 'var(--color-danger)' },
    { key: 'late', label: 'متأخر', value: late, sub: '', icon: 'solar:clock-circle-bold-duotone', color: 'var(--color-warning)' },
    { key: 'excused', label: 'إعفاء/مُبرر', value: excused, sub: '', icon: 'solar:shield-check-bold-duotone', color: 'var(--color-secondary)' },
    { key: 'runaway', label: 'هروب', value: runaway, sub: '', icon: 'solar:running-bold-duotone', color: 'var(--color-danger)' },
    { key: 'exit_open', label: 'إذونات خروج (مفتوحة)', value: exitOpen, sub: exitTotal ? `الإجمالي ${exitTotal}` : '', icon: 'solar:logout-3-bold-duotone', color: 'var(--color-info)' },
    { key: 'total', label: 'إجمالي الطلاب', value: total, sub: '', icon: 'solar:users-group-rounded-bold-duotone', color: 'var(--color-secondary)' },
  ];
});
</script>
<style scoped>
.header-icon { font-size: 26px; color: var(--maron-primary); }
...
.stat-card {
  ...
  gap: .2rem;
  padding: .5rem !important; /* تقليل الحشوة لمستوى أدنى */
}
/* تصغير الأيقونة داخل البطاقة */
.stat-card :deep(svg) { font-size: clamp(0.95rem, 1.4vw, 1.25rem); }
/* قيمة المؤشر صغيرة قدر الإمكان مع الحفاظ على الوضوح */
.stat-card .value { line-height: 1; font-weight: 700; font-size: clamp(1rem, 1.6vw, 1.35rem); }
.stat-card .label { white-space: nowrap; text-overflow: ellipsis; overflow: hidden; max-width: 100%; font-size: .72rem; }
.stat-card .sub { max-width: 100%; white-space: nowrap; text-overflow: ellipsis; overflow: hidden; color: #6c757d; font-size: .7rem; }

/* تأثير طفيف للاحترافية */
.stat-card:hover { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(0,0,0,.08); transition: transform .15s ease, box-shadow .15s ease; }

/* شريط أدوات تحديد الصفوف */
.class-toolbar .btn { padding: .15rem .5rem; }
</style>
