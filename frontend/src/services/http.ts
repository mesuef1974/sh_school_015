...
function createClient(opts: HttpOptions = {}): AxiosInstance {
  ...
  instance.interceptors.response.use(
    ...
    async (error: AxiosError) => {
      ...
      const original = (error.config || {}) as AxiosRequestConfig & { _retry?: boolean; _retryCount?: number }
      ...
      if (isTransient) {
        ...
        if (original._retryCount <= maxRetries) {
          ...
          logDebug(`Transient error (status=${status || 'net'}). Retry ${original._retryCount}/${maxRetries} after ${delay}ms`, original.url)
          await sleep(delay)
          return instance(original)
        }
      }
