<template>
  <span :class="['ds-icon', sizeClass, { block }]" :style="styleVars" role="img" :aria-label="ariaLabel">
    <component v-if="isIconify" :is="IconifyIcon" :icon="icon" />
    <span v-else v-html="svgHtml" aria-hidden="true"></span>
  </span>
</template>
...
const ariaLabel = computed(() => props.label || name.value);
...
onMounted(() => { ensureSchoolSvg(); });
watchEffect(() => { ensureSchoolSvg(); });
</script>

<style scoped>
.ds-icon { display: inline-flex; align-items: center; justify-content: center; line-height: 1; color: var(--icon-color, currentColor); }
.ds-icon > span:deep(svg), .ds-icon :deep(svg) { width: var(--icon-size, 1em); height: var(--icon-size, 1em); display: block; }
/* Standard sizes */
.ds-icon.text-sm { --icon-size: 14px; }
.ds-icon.text-base { --icon-size: 16px; }
.ds-icon.text-lg { --icon-size: 20px; }
.ds-icon.text-xl { --icon-size: 24px; }
</style>