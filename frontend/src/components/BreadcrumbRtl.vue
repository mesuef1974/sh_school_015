...
function labelFromRoute(r: RouteLocationNormalizedLoaded | any): string {
  return (
    (r.meta?.titleAr as string) ||
    (r.meta?.title as string) ||
    (r.name as string) ||
    (r.path as string)
  );
}
...
function saveHistory() {
  try {
    sessionStorage.setItem(STORAGE_KEY, JSON.stringify(historyState.items));
  } catch {}
}
...
const allOptions = computed<Crumb[]>(() => {
  return router
    ...
    .map((r) => ({
      key: r.path,
      label: (r.meta as any)?.titleAr || (r.name as string) || r.path,
      href: r.path,
      path: r.path,
    }));
});
...
const filteredOptions = computed(() => {
  ...
  return allOptions.value
    .filter((o) => (o.label || "").toString().toLowerCase().includes(lower))
    .slice(0, 8);
});
...
.breadcrumb__option.is-active {
  background: rgba(255, 255, 255, 0.08);
}
.breadcrumb__option-path {
  opacity: 0.75;
}
