...
> - أين وصلنا الآن:
>
> تحديث موجز — صفحات وبطاقات مشرف الجناح (Wing Supervisor) — 2025-10-31 00:23
>
> الملخص: الخطة جاهزة ومفصلة في DOC/WingSupervisor_TestPlan.md. التنفيذ الكامل لم يبدأ بعد؛ المطلوب الآن فتح مهام فرعية وتنفيذ API + واجهة + اختبارات كما هو موضح في الخطة.
> - لوحة الغياب اليومية (/wing/attendance/daily): الحالة: لم تُنفّذ وظيفيًا بعد؛ المتبقي: [UT-Daily-Policy]، [IT-Overview-200]، [IT-DailyAbs-200]، [IT-Entered-200]/[IT-Missing-200]، [E2E-Daily-Cards]، [AX-Daily-Buttons].
> - طلبات الاعتماد (/wing/approvals): الحالة: غير منفّذة؛ المتبقي: [UT-Approvals-RBAC]، [IT-Pending-200]، [IT-Decide-200]، [IT-Excused-200]، [E2E-Approvals-Flow]، [AX-Approvals-Table].
> - إدارة الغيابات وتنبيهات الغياب (/wing/absences): الحالة: غير منفّذة؛ المتبقي: [UT-Alerts-Numbering]، [IT-ComputeDays-200]، [IT-Alerts-Create-200]، [IT-Alerts-Docx]، [E2E-Alerts-Issue]، [AX-Alerts-Form].
> - CSV (اختياري): مؤجل حتى اكتمال الأساسيات؛ [IT-CSV-Exports]، [E2E-CSV-Buttons].
> - الأداء والموثوقية: يُعالج بعد الأساسيات؛ [UT-Perf-Cap]، [IT-Overview-Cache].
> - الوصولية العامة (A11y): [AX-Smoke] على الصفحات الثلاث بعد الدمج الأولي.
>
> الخطوات التالية المقترحة (تنفيذية):
> 1) Backend: إعداد ViewSets ونقاط API المذكورة مع RBAC مشرف الجناح، والبدء باختبارات وحدات للسياسات الأساسية.
> 2) Frontend: إنشاء الصفحات الثلاث بالحد الأدنى القابل للاختبار، مع StatusLegend وتوحيد الشارات.
> 3) تكامل API: تغطية GET/POST الأساسية بحسب بنود الخطة.
> 4) E2E: سيناريو دخاني لكل صفحة ثم توسيعه تدريجيًا.
> 5) A11y: تسميات أزرار/حقول واضحة + axe smoke.
> 6) CSV: تمكين التصدير عند الحاجة بعد اكتمال البيانات الأساسية.
...
## تحديث — 2025-10-31 00:40 — تحسين أيقونات المواد في الجدول اليومي/الأسبوعي
- تم توسيع خريطة أيقونات المواد frontend/src/shared/icons/subjectIcons.ts لتغطية مواد لم تكن تملك أيقونات:
  - إضافة قواعد: رياض الأطفال/الطفولة المبكرة، فلسفة/منطق/تفكير نقدي.
  - تصحيح ترتيب القواعد: نقل «علم النفس» قبل مجموعة «علوم» لمنع التقاطه بشكل خاطئ.
  - تحسين مجموعة المسميات لعدد من المواد (مهن/هندسي/ورش) واختيار أيقونات أكثر تعبيرًا.
- أماكن الاستخدام المتأثرة:
  - TeacherTimetable.vue، WingTimetable.vue، TeacherAttendanceHistory.vue، WingDashboard.vue.
- الأثر: تظهر الآن أيقونات معبّرة لعدد أكبر من المواد؛ تبقى أيقونة افتراضية واضحة عند غياب تطابق لضمان عدم ظهور خلايا بلا رمز.

## تحديث — 2025-10-31 00:46 — معالجة «علوم الحاسب» والمواد بلا أيقونات
- تم تعزيز المطابقة لتجنّب تصنيف «علوم الحاسب/الحاسوب/الكمبيوتر» كـ «علوم» عامة:
  - أُضيفت قاعدة عالية الأولوية تتعرف على الصيغ: «علوم الحاسب/علم الحاسب/علوم الحاسوب/علوم الكمبيوتر/حاسب آلي/Computer Science/CS/تقنية المعلومات/تكنولوجيا المعلومات/نظم معلومات/Information Technology» وتعرض أيقونة لابتوب.
  - تم تعديل قاعدة «علوم» العامة لتستثني صراحة تراكيب «علوم الحاسب/الحاسوب/الكمبيوتر» حتى لا تطغى على القاعدة المتخصصة.
- أُضيفت آلية تشخيصية اختيارية (dev-only) لاكتشاف المواد التي تقع على fallback:
  - ضع VITE_DEBUG_SUBJECT_ICONS=true في frontend/.env أثناء التطوير لعرض console.warn عند استخدام fallback مع إظهار النص الخام بعد التطبيع.
- النتيجة المتوقعة:
  - تظهر أيقونة الحاسوب للمواد: «علوم الحاسب»، «حاسب آلي»، «Computer Science»، «تقنية المعلومات»، إلخ.
  - انخفاض ملحوظ في عدد الخلايا التي تظهر الأيقونة الافتراضية.
- في حال وجود تسميات خاصة بمدرستكم، يُفضّل تزويدنا بقائمة الأسماء كما هي في قاعدة البيانات لنضيف مرادفات دقيقة.

## تحديث — 2025-10-31 00:52 — إصلاح بقاء «جناح 5» واسم مشرفه عالقًا عند التبديل
- السبب: مكوّن الترويسة على صفحات «مشرف الجناح» كان يعرض دائمًا «الجناح الأساسي» القادم من API (`primary_wing`) بغضّ النظر عن الجناح المختار داخل الصفحات (مثل «جدول الجناح»). لذا بقيت الترويسة تُظهر بيانات جناح 5.
- الإصلاح البرمجي:
  - frontend/src/shared/composables/useWingContext.ts: إضافة حالة تفاعلية `selectedWingId/Name` ودوال `setSelectedWing` و`clearSelectedWing` وجعل الترويسة تفضّل الجناح المختار إن وُجد، مع إظهار اسم المشرف فقط إذا كان الجناح المختار هو الجناح الأساسي نفسه.
  - frontend/src/features/wings/pages/WingTimetable.vue: عند تحميل/تغيير الاختيار في قائمة الأجنحة، يتم استدعاء `setSelectedWing` لتحديث الترويسة عبر الصفحات.
  - تم أيضًا استدعاء `ensureLoaded()` في الصفحات الجديدة Placeholder لضمان تحميل سياق الجناح.
- النتيجة: عند تبديل الجناح من قائمة «جدول الجناح»، تتحدّث الترويسة فورًا ولا تبقى عالقة على جناح 5. كما أن تبديل الحساب (تسجيل الخروج) يمسح السياق منعًا لأي تسرّب.

## تحديث — 2025-10-31 01:01 — استخراج قائمة المواد بلا أيقونات معبّرة (تشخيص)
- الهدف: تزويدك بقائمة «المواد التي لا تملك أيقونات معبّرة» كما تظهر فعليًا في بيئتك.
- ما أضفناه (FRONTEND فقط):
  - تعزيز تشخيص subjectIcons: عند ضبط VITE_DEBUG_SUBJECT_ICONS=true يتم جمع كل التسميات التي تقع على الأيقونة الافتراضية (fallback) داخل مجموعة داخلية قابلة للعرض.
  - صفحة أدوات جديدة: /debug/subject-icons تعرض قائمتين: النصوص الأصلية (Raw) والنصوص بعد التطبيع (Normalized) مع أزرار نسخ/مسح.
- كيفية الاستخدام سريعًا:
  1) في frontend/.env ضع: VITE_DEBUG_SUBJECT_ICONS=true
  2) شغّل الواجهة ثم تفقّد الصفحات التي تُظهر المواد (جدول المعلم، جدول الجناح اليومي/الأسبوعي، السجل…)
  3) افتح /debug/subject-icons وانسخ القائمة. يمكنك مسحها وإعادة التجميع بعد كل جولة.
- ملاحظات:
  - هذا تشخيص للواجهة فقط؛ لا يلمس قواعد الأيقونات الحالية ولا يؤثر على الإنتاج إن بقي العلم = false (افتراضي).
  - عند مشاركتنا القائمة، سنضيف قواعد دقيقة لتغطية هذه المواد بأيقونات معبّرة.